...
export const TradingChart = ({ symbol, onPriceUpdate }: TradingChartProps) => {
  const [priceHistory, setPriceHistory] = useState<PriceData[]>([]);
  const [realtimePrice, setRealtimePrice] = useState<RealtimePrice | null>(null);
  const [loading, setLoading] = useState(true);
  const [timeframe, setTimeframe] = useState<'5m' | '15m' | '1h' | '4h' | '1d'>('5m');
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const history = await getPriceHistory({ symbol, interval: timeframe, start }, controller.signal);
    ...
    const pollId = window.setInterval(async () => {
      try {
        ...
        try {
          ...
          const latest = await getPriceHistory({ symbol, interval: timeframe, start: start2 }, controller.signal);
          ...
          {realtimePrice && (
            <div>
              ...
              <div className="w-full flex justify-end">
                <Select value={timeframe} onValueChange={(v: string) => setTimeframe(v as '5m' | '15m' | '1h' | '4h' | '1d')}>
